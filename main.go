...
func globalAdminsFromEnv() []string {
	...
	return trimmedUuids
	// return models.Group{Name: "globalAdmins", Members: trimmedUuids}
}
...
func main() {
	...
	myRouter.DefaultMentionRoute = *fallback.GetMentionRoute()
	myRouter.DeniedMentionRoute = *permission_denied.GetMentionRoute()
	myRouter.AddMentionRoutes(how.GetMentionRoutes())
	myRouter.AddMentionRoutes(groups.GetMentionRoutes())
	myRouter.AddMentionRoutes(user_info.GetMentionRoutes())
	...
	http.HandleFunc("/gadget", func(w http.ResponseWriter, r *http.Request) {
		...
		if eventsAPIEvent.Type == slackevents.CallbackEvent {
			...
			case *slackevents.AppMentionEvent:
				...
				db.FirstOrCreate(&currentUser, models.User{Uuid: ev.User})
				...
				route, exists := myRouter.FindMentionRouteByMessage(trimmedMessage)
				if !exists {
					route = myRouter.DefaultMentionRoute
				}
				if !myRouter.Can(currentUser, route.Permissions) {
					log.Warn().Str("user", currentUser.Uuid).Str("route", route.Name).Msg("Permission failure")
					route = myRouter.DeniedMentionRoute
				}
				...
				go route.Execute(*api, *myRouter, *ev, trimmedMessage)
			}
		}
	})
